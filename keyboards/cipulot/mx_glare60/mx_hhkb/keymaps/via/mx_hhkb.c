...
void eeconfig_init_kb(void) {
    ...
    for (int i = 0; i < 4; i++) {
        eeprom_mx_hhkb_config.socd_opposing_pairs[i].keys[0]    = socd_pairs[i].key1;
        eeprom_mx_hhkb_config.socd_opposing_pairs[i].keys[1]    = socd_pairs[i].key2;
        eeprom_mx_hhkb_config.socd_opposing_pairs[i].resolution = SOCD_CLEANER_OFF;
        eeprom_mx_hhkb_config.socd_opposing_pairs[i].held[0]    = false;
        eeprom_mx_hhkb_config.socd_opposing_pairs[i].held[1]    = false;
    }
...
void keyboard_post_init_kb(void) {
    ...
    memcpy(socd_opposing_pairs, eeprom_mx_hhkb_config.socd_opposing_pairs, sizeof(socd_opposing_pairs));
