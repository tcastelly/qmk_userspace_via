    ...
    if ((*value_id) < id_socd_pair_1_mode) {
        ...
        switch (data_index) {
            case 1: {
                ...
                if (indi_index == 0) {
                    eeconfig_update_kb_datablock_field(eeprom_mx_hhkb_config, ind1.enabled);
                } else if (indi_index == 1) {
                    eeconfig_update_kb_datablock_field(eeprom_mx_hhkb_config, ind2.enabled);
                }
                break;
            }
            case 2: {
                ...
                if (indi_index == 0) {
                    eeconfig_update_kb_datablock_field(eeprom_mx_hhkb_config, ind1.v);
                } else if (indi_index == 1) {
                    eeconfig_update_kb_datablock_field(eeprom_mx_hhkb_config, ind2.v);
                }
                break;
            }
            case 3: {
                ...
                if (indi_index == 0) {
                    eeconfig_update_kb_datablock_field(eeprom_mx_hhkb_config, ind1.h);
                    eeconfig_update_kb_datablock_field(eeprom_mx_hhkb_config, ind1.s);
                } else if (indi_index == 1) {
                    eeconfig_update_kb_datablock_field(eeprom_mx_hhkb_config, ind2.h);
                    eeconfig_update_kb_datablock_field(eeprom_mx_hhkb_config, ind2.s);
                }
                break;
            }
            case 4: {
                ...
                if (indi_index == 0) {
                    eeconfig_update_kb_datablock_field(eeprom_mx_hhkb_config, ind1.func);
                } else if (indi_index == 1) {
                    eeconfig_update_kb_datablock_field(eeprom_mx_hhkb_config, ind2.func);
                }
                break;
            }
            default: {
    ...
    if (mode) { // set
        switch (field) {
            case 0: // mode/resolution
                ...
                eeprom_mx_hhkb_config.eeprom_socd_opposing_pairs[pair_idx].resolution = value;
                eeprom_mx_hhkb_config.eeprom_socd_opposing_pairs[pair_idx].held[0]    = false;
                eeprom_mx_hhkb_config.eeprom_socd_opposing_pairs[pair_idx].held[1]    = false;
                eeconfig_update_kb_datablock_field(eeprom_mx_hhkb_config, eeprom_socd_opposing_pairs);
                return 0;
            case 1: // key 1
                socd_opposing_pairs[pair_idx].keys[0]                              = value;
                socd_opposing_pairs[pair_idx].held[0]                              = false;
                socd_opposing_pairs[pair_idx].held[1]                              = false;
                eeprom_mx_hhkb_config.eeprom_socd_opposing_pairs[pair_idx].keys[0] = value;
                eeprom_mx_hhkb_config.eeprom_socd_opposing_pairs[pair_idx].held[0] = false;
                eeprom_mx_hhkb_config.eeprom_socd_opposing_pairs[pair_idx].held[1] = false;
                eeconfig_update_kb_datablock_field(eeprom_mx_hhkb_config, eeprom_socd_opposing_pairs);
                return 0;
            case 2: // key 2
                ...
                socd_opposing_pairs[pair_idx].held[0]                              = false;
                socd_opposing_pairs[pair_idx].held[1]                              = false;
                eeprom_mx_hhkb_config.eeprom_socd_opposing_pairs[pair_idx].keys[1] = value;
                eeprom_mx_hhkb_config.eeprom_socd_opposing_pairs[pair_idx].held[0] = false;
                eeprom_mx_hhkb_config.eeprom_socd_opposing_pairs[pair_idx].held[1] = false;
                eeconfig_update_kb_datablock_field(eeprom_mx_hhkb_config, eeprom_socd_opposing_pairs);
                return 0;
        ...
        switch (field) {
            case 0: // mode/resolution
                return eeprom_mx_hhkb_config.eeprom_socd_opposing_pairs[pair_idx].resolution;
            case 1: // key 1
                return eeprom_mx_hhkb_config.eeprom_socd_opposing_pairs[pair_idx].keys[0];
            case 2: // key 2
                return eeprom_mx_hhkb_config.eeprom_socd_opposing_pairs[pair_idx].keys[1];
            default:
                return 0;
        }
    }
