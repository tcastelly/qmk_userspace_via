...
uint16_t socd_pair_handler(bool mode, uint8_t pair_idx, uint8_t field, uint16_t value) {
    if (mode) { // set
        switch (field) {
            case 0: // enabled
                eeprom_mx_wkl_config.socd_opposing_pairs[pair_idx].resolution = value;
                socd_opposing_pairs[pair_idx].resolution                  = value;
                eeconfig_update_kb_datablock_field(eeprom_mx_wkl_config, socd_opposing_pairs);
                return 0;
            case 1: // key 1
                eeprom_mx_wkl_config.socd_opposing_pairs[pair_idx].keys[0] = value;
                socd_opposing_pairs[pair_idx].keys[0]                  = value;
                eeconfig_update_kb_datablock_field(eeprom_mx_wkl_config, socd_opposing_pairs);
                return 0;
            case 2: // key 2
                eeprom_mx_wkl_config.socd_opposing_pairs[pair_idx].keys[1] = value;
        ...
        switch (field) {
            case 0:
                return eeprom_mx_wkl_config.socd_opposing_pairs[pair_idx].resolution;
            case 1:
                return eeprom_mx_wkl_config.socd_opposing_pairs[pair_idx].keys[0];
            case 2:
                return eeprom_mx_wkl_config.socd_opposing_pairs[pair_idx].keys[1];
            case 3:
                return eeprom_mx_wkl_config.socd_opposing_pairs[pair_idx].resolution;
            default:
                return 0;
