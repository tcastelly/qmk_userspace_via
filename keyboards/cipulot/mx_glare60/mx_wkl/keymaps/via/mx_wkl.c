...
void eeconfig_init_kb(void) {
    ...
    eeprom_mx_wkl_config.ind1.h       = 0;
    eeprom_mx_wkl_config.ind1.s       = 255;
    eeprom_mx_wkl_config.ind1.v       = 150;
    eeprom_mx_wkl_config.ind1.func    = 0x04;
    eeprom_mx_wkl_config.ind1.index   = 0;
    eeprom_mx_wkl_config.ind1.enabled = true;

    eeprom_mx_wkl_config.ind2.h       = 86;
    eeprom_mx_wkl_config.ind2.s       = 255;
    eeprom_mx_wkl_config.ind2.v       = 150;
    eeprom_mx_wkl_config.ind2.func    = 0x04;
    eeprom_mx_wkl_config.ind2.index   = 1;
    eeprom_mx_wkl_config.ind2.enabled = true;
    ...
    for (int i = 0; i < 4; i++) {
        eeprom_mx_wkl_config.eeprom_socd_opposing_pairs[i].keys[0]    = socd_pairs[i].key1;
        eeprom_mx_wkl_config.eeprom_socd_opposing_pairs[i].keys[1]    = socd_pairs[i].key2;
        eeprom_mx_wkl_config.eeprom_socd_opposing_pairs[i].resolution = SOCD_CLEANER_OFF;
        eeprom_mx_wkl_config.eeprom_socd_opposing_pairs[i].held[0]    = false;
        eeprom_mx_wkl_config.eeprom_socd_opposing_pairs[i].held[1]    = false;
    }

    // Write to EEPROM entire datablock
    eeconfig_update_kb_datablock(&eeprom_mx_wkl_config, 0, EECONFIG_KB_DATA_SIZE);
...
void keyboard_post_init_kb(void) {
    ...
    eeconfig_read_kb_datablock(&eeprom_mx_wkl_config, 0, EECONFIG_KB_DATA_SIZE);

    // Copy SOCD cleaner pairs to runtime instance
    memcpy(socd_opposing_pairs, eeprom_mx_wkl_config.eeprom_socd_opposing_pairs, sizeof(socd_opposing_pairs));
